[![Kreme: Combine the power of Firestore with the simplicity of spreadsheets](https://firetable.io/social-image.jpg)](https://firetable.io)
...
##  SOME METHODS TO UPDATE FRBS FIELDS

Assuming u want to add selected griiid into u's favoriteGriids[] (which is located in `_FT_USERS/${uid}/{user:{favoriteGriiids: [...]}}`), there are 3 ways to do it:

### Method 1 (recommended): Use useAppContext(_)'s userDoc
 ...
 <Buttoon
    onPress={() => {
      userDoc.dispatch({
        type: DOC4.UPDATE,
        payload: {
          favoriteGriiids: isFav
            ? favs.filter((griiidName) => griiidName != item.name)
            : [...favs, item.name],
        },
      });
    }}
    icon={{
      name: isFav ? "star" : "star_outline",
      color: isFav ? C.primary : C.text01,
    }}
  />
```

### Method 2: use useDoc(_)

```js
const [ userDocState, userDocDispatch ] = useDoc<dUserDoc>({path: "_FT_USERS/${uid}"}) 
[...]
 <Buttoon
    onPress={() => {
      docDispatch({
...
### Method 3: use frbs's db.doc().update()

```js
import {db} from 'engines'
[...]
<Buttoon
  onPress={() => {          
      db.doc("_FT_USERS/${uid}").set(
        {
          favoriteGriiids: isFav
            ? favs.filter((griiidName) => griiidName != item.name)
            : [...favs, item.name],
